import { c as createComponent, m as maybeRenderHead, r as renderTemplate, b as createAstro, e as addAttribute, f as renderHead, g as renderComponent, h as renderSlot, s as spreadAttributes, F as Fragment } from '../chunks/astro/server_BduYY49k.mjs';
import 'kleur/colors';
import 'clsx';
/* empty css                                 */
...
const $$Projects = createComponent(($$result, $$props, $$slots) => {
  ...
  const PROJECTS = [
    {
      title: "Slack Version",
      description: "Basado en Slack con dise\xF1o propio que implica crear una plataforma de comunicaci\xF3n similar a Slack, pero con un dise\xF1o personalizado...",
    ...
    },
    {
      ...
      description: "Este proyecto tiene como objetivo recrear la interfaz de usuario de Spotify, enfoc\xE1ndose en su dise\xF1o limpio, funcional y moderno...",
  ...
  return renderTemplate`${maybeRenderHead()}<section> ${PROJECTS.map(({ title, description, link, github, image, tags }) => renderTemplate`<article class="mt-8 overflow-hidden"> <h3 class="text-2xl font-semibold text-blue-400 mb-2"> ${title} </h3> <p class="text-lg mb-4 text-pretty">${description}</p> <ul class="flex flex-wrap gap-x-2 flex-row"> ${tags.map((tag) => renderTemplate`<li class="flex"> ${tag.name === "GitHub" || tag.name === "Demo" ? renderTemplate`<a${addAttribute(tag.name === "GitHub" ? github : link, "href")} target="_blank" rel="noopener noreferrer"${addAttribute(`flex gap-x-2 text-xs rounded-full items-center ${tag.class} my-2 py-2 px-2 hover:scale-105 transition-transform`, "class")}> ${renderComponent($$result, "tag.icon", tag.icon, { "class": "size-6" })} ${tag.name} </a>` : renderTemplate`<span${addAttribute(`flex gap-x-2 text-xs rounded-full items-center ${tag.class} my-2 py-2 px-2`, "class")}> ${renderComponent($$result, "tag.icon", tag.icon, { "class": "size-6" })} ${tag.name} </span>`} </li>`)} </ul> <img class="rounded shadow-2xl hover:scale-102 transition-transform shadow-white/10"${addAttribute(image, "src")}${addAttribute(`Captura del Proyecto ${title}`, "alt")}> </article>`)} </section>`;
